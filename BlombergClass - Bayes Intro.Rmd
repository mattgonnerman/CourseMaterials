...
More in depth options for model diagnostics exist, but I will only present a few below for you to [explore yourself](https://go.documentation.sas.com/doc/en/pgmsascdc/9.4_3.3/statug/statug_introbayes_sect025.htm). 
...
### Comparing Bayesian and Frequentist Approaches
There is much to debate about the efficacy of Bayesian approaches over Frequentist, so lets just break down the most glaring similarities and differences between the two to see why we would choose one over the other.  

**Ease of Use** - The simple fact is, in most cases the Frequentist approach to analysing a model will be simpler and easier than a Bayesian equivalent. For simple models, it is unlikely that you will need to worry much about unique specifications or unsupported additions, in which case the pre-prepared functions provided by a package will likely get you to where you need to go. But once you begin to introduce complexity into your model or need to perform novel analysis, you will begin to be more limited by the packages available to use. This is where a Bayesian approach will become more enticing. Yes, you will need to put in some effort into generating a model and running the MCMC, but you already have the tools available to perform such analysis. Should you go down the route of customized frequentist analyses, you may find yourself limited by your coding or mathematical skills.  

**Computational Limitations** - Bayesian simulations also tend to require greater computational power to run and store all of the simulations that you are running. This means more total time your computer needs to be running as well as a need for greater storage space. You can get around some of these limitations by using more advanced samplers or adjusting MCMC specifications, but it will likely still exceed the requirements of a Frequentist approach.  

**Interpretation of Uncertainty** - One argument for a Bayesian approach is a more intuitive interpretation of model uncertainty. In a Frequentist approach, "all probabilities are defined by their connection to countable events and their frequency in very large samples" (McElreath - Statistical Rethinking). This leads to a rather obtuse interpretation where uncertainty is premised on theoretical re-samples of the data which never occur. In contrast, a Bayesian approach returns a posterior distribution which can be used to make probabilistic statements about our estimates and their uncertainty.  

**Integrating Models/Propogation of Uncertainty** - The flexibility of a Bayesian approach lends itself to customization and complexity, which is especially useful when linking multiple models together. In ecology, we frequently deal with related models that share parameters which we wish to estimate. When models share the same or similar parameters, we may wish to integrate the models to share information among them and improve inference overall. While this is possible to do in a Frequentist setting, it is much easier to do within a single Bayesian model largely because parameter estimates are represented  probability distributions which can be used as priors for other parameters. This is just one way in which model estimates can be linked and uncertainty can be translated directly between models.

#### Direct Comparison of RMark and R2JAGS results
Despite their differences, given the same or similar models, Bayesian and Frequentist approaches will usually lead to the same conclusion. This is because many of the differences lie in interpretation and implementation, not on the formulas defining the relationships. To demonstrate this, let's quickly run the exact same data we simulated for JAGS through RMark to see how results change. 

First we will need to reformat our data once again, this time making sure it is suitable for use in MARK. For details on what that encounter history should look like, you can check out [chapter 17 of the MARK manual](http://www.phidot.org/software/mark/docs/book/pdf/chap17.pdf).
