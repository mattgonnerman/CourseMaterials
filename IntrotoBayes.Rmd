...
#### Bayes Theorem:
Moving on from the philosophical differences in how we describe results, let's discus how a bayesian approach actually differs in practice. To understand this, we will need to to use probability theory. We can approach our question by considering the probability of our data and $\theta$ occurring together, otherwise written as $P(y, \theta)$. This probability can be rederived and written a number of different ways, shown below...   

$$ P(y, \theta) = Pr(\theta|y)Pr(y) = Pr(y|\theta) Pr(\theta)$$  

When we breakdown these probability statements, we find that the probability of both y and $\theta$ occurring together is the same as the probability of y occurring multiplied by the probability of $\theta$ occurring given that y also occurs (or vice versa). Using basic algebra, we can rearrange our equation to create the below statement, also referred to as Bayes Theorem.  
...
So now that you have at least a broad understanding of the differences in philosophy between the two approaches, lets get into more detail on the similarities and differences between them.
