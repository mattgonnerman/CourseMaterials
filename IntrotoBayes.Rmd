...
So now we have our data and the research question we wish to answer, but we still need to determine how we will approach our analysis (ideally this is done in the planning stage of your research). Within the field of ecology, we tend to choose an approaches that fall within one of two paradigms, bayesian or frequentist statistics. While there are many similarities and distinctions between the two (which we will summarize at the end of this module), let's take a 10,000 ft view of how each approaches known and unknown information.
...
A Bayesian approach is going to start in a very similar place as a frequentist one. We have known data (y), a parameter we wish to describe ($\theta$), and a model defining our hypothesized relationship between the two. Where bayesian deviates from frequentist is in how we treat $\theta$. Instead of seeking to define a fixed value around which there is some uncertainty (i.e. Frequentist), we instead are looking to define some probability distribution that describes [A key distinction here is that we are not seeking some fixed value of ] We can approach this problem by considering the probability of our data and $\theta$ occuring together or $P(y, \theta)$.  
...
So now that you have at least a broad understanding of the differences in philosophy between the two approaches, lets identify the aspects of each that are similar and different from one another. 
