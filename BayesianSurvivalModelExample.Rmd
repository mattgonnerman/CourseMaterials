...
w.days <- 60  #the length of winter
...
```{r}
n.turkey <- 100  #number of simulated turkeys
...
```{r}
obs.rate <- .90 #We can't expect to observe each turkey every day, so this just randomizes whether we successfully collect data on a bird on a given day. Let's assume that on average we find a bird every 4 days.
