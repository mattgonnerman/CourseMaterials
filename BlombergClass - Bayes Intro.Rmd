...
### Building Our Survival Model  
Up to this point, we have explicitly stated our research question and hypotheses which we wish to test. We have also translated those hypotheses into a statistical model which we need to translate further into code that can be read by R. This will explicitly define how our data is used within the model and therefore determine how it needs to be collected and formatted for the model to function appropriately. Considering we have walked through the model construction, I will present the final model in the appropriate JAGS coding 
data in the form of daily survival checks and individual-specific body condition and habitat use information which constitutes our known information. We have also broadly identified our unknown information which we wish to estimate, [WHICH ARE???]. So the next step is to create our model, which is where we describe the relationship between our known and unknown data. 
